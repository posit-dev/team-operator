...
// Workbench site. When this struct is present on a WorkbenchSpec, the
// controller is active for that site; when absent, the site is skipped.
//
// Example site.yaml:
//
//	workbench:
...
type SessionLabelsConfig struct {
	// SourceField is a dot-path into the pod spec that identifies the field
	// holding the comma-separated group list. Supports array index notation.
	// Default: "spec.containers[0].args"
	// Examples:
	//   "spec.containers[0].args"  — container args slice (lookup by sourceKey flag)
	//   "metadata.annotations"     — all annotations (lookup by sourceKey map key)
	//   "metadata.labels"          — all labels (lookup by sourceKey map key)
	// +optional
	SourceField string `json:"sourceField,omitempty"`

	// SourceKey is the sub-key used to extract the final string from the resolved field:
	//   []string field (e.g. args): the flag name whose next element is the value.
	//   map field (e.g. annotations, labels): the map key to look up.
	//   string field: ignored; the field value is used directly.
	// Default: "--container-user-groups"
	// +optional
	SourceKey string `json:"sourceKey,omitempty"`

	// SearchRegex is applied to each comma-separated entry in the source value to
	...
	// Default "user-group-" produces labels user-group-1, user-group-2, etc.
	// +optional
	LabelKeyPrefix string `json:"labelKeyPrefix,omitempty"`

	// TrimPrefix is stripped from the start of each matched group name before it
	// becomes the label value. E.g. with TrimPrefix "_": "_entra_team" → "entra_team".
	// Default: "_"
	// +optional
