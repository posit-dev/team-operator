...
{{- range $parent, $child := . -}}
  ...
  {{- range $i, $section := $sections -}}
    {{- if kindIs "map" $section }}
      {{- if not ( kindIs "slice" . ) -}}
        {{- printf "[%s]" (toString $parent) | nindent 0 }}
      {{- end }}
      {{- range $key, $val := $section }}
        {{- printf "%s=%s" (toString $key) (toString $val) | nindent 0 }}
      {{- end }}
      {{- printf "" | nindent 0 }}
    {{- else }}
      {{- printf "%s=%s" (toString $parent) (toString $section) | nindent 0 }}
    {{- end }}
  {{- end }}
{{- end }}
{{- end }}
...
{{- define "rstudio-library.config.json-entry" -}}
{{ . | toPrettyJson }}
{{- end -}}

{{- define "rstudio-library.config.newline-entry" -}}
{{- range $tmp := . }}
{{ . }}
{{- end }}
{{- "\n" -}}
