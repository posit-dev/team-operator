...
func TestPhaseFromConditions(t *testing.T) {
	cases := []struct {
		...
		{"deployment not ready reason", []metav1.Condition{{Type: status.TypeReady, Reason: status.ReasonDeploymentNotReady}}, observability.PhaseProgressing},
		{"statefulset not ready reason", []metav1.Condition{{Type: status.TypeReady, Reason: status.ReasonStatefulSetNotReady}}, observability.PhaseProgressing},
