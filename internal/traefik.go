...
func DeployTraefikForwardMiddleware(ctx context.Context, req ctrl.Request, r product.SomeReconciler, name string, p product.KubernetesOwnerProvider) error {
	l := r.GetLogger(ctx).WithValues(
		"function", "DeployTraefikForwardMiddleware",
	)
	forwardMiddleware := &v1alpha1.Middleware{
	...
	existingForwardMiddleware := &v1alpha1.Middleware{}
	forwardMiddlewareKey := client.ObjectKey{Name: name, Namespace: req.Namespace}
	l.Info("CREATING Forward traefik middleware...")
	if err := BasicCreateOrUpdate(ctx, r, l, forwardMiddlewareKey, existingForwardMiddleware, forwardMiddleware); err != nil {
...
func DeployTraefikForwardMiddlewareWithHost(ctx context.Context, req ctrl.Request, r product.SomeReconciler, name string, p product.KubernetesOwnerProvider, host string) error {
	l := r.GetLogger(ctx).WithValues(
